...
class FeatureExtractionVertical(object):
    ...
    def init_node(self, ns="~"):
        ...
        if self.compressed_images:
            self.sonar_sub = rospy.Subscriber(
                SONAR_VERTICAL_TOPIC, OculusPing, self.callback, queue_size=1)
        else:
            self.sonar_sub = rospy.Subscriber(
        ...
        self.feature_pub = rospy.Publisher(
            SONAR_FEATURE_VERTICAL_TOPIC, PointCloud2, queue_size=1)

        #vis publish topic
        self.feature_img_pub = rospy.Publisher(
            SONAR_FEATURE_VERTICAL_IMG_TOPIC, Image, queue_size=1)
    ...
    def publish_features(self, ping, points):
        ...
        points = np.c_[points[:,0],  np.zeros(len(points)), -points[:,1]] # With Kalman filter
